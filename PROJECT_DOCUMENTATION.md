...
| **Time Steps** | 8 per circuit |
| **Gates Implemented** | 15+ total (H, X, Y, Z, S, T, CNOT, SWAP, Measurement) |
| **Pre-built Templates** | 10+ educational circuits |
| **Documentation** | 5+ comprehensive guides |
| **Code Style** | Pedagogical with extensive comments |
...
```
Backend (C++)
├── Quantum_registers_cl.cpp
│   ├── State vector representation (complex amplitudes)
│   ├── Normalization logic
│   ├── Measurement mechanics
│   └── Quantum state initialization
├── RegisterGates_cl.cpp
│   ├── Hadamard gate implementation
│   ├── Pauli gates (X, Y, Z)
│   ├── Phase gates (S, T)
│   └── Register-level gate operations
├── Gates_cl.cpp
│   ├── Single-qubit gate definitions
│   ├── Matrix representations
│   └── Amplitude evolution rules
└── DeutscheAlgo_example.cpp
...
#### 1. **Quantum_registers_cl.cpp** - Core State Management
**Purpose**: Foundation of quantum state representation

```cpp
Key Concepts:
- std::vector<std::complex<double>> for state vector
- Amplitude storage: index = binary representation of qubit states
- Example: state[5] = amplitude of |101⟩ for 3 qubits

Key Functions:
- QuantumRegister(int n_qubits) - Initialize n-qubit state
- void normalize() - Ensure sum of |amplitude|² = 1
- double measureProbability(int outcome) - Get P(outcome)
- std::string measure() - Collapse and return n-bit string
- std::string peekMeasurement() - Sample without collapse
```

**Educational Value**: Shows exactly how quantum states are represented and evolved.

#### 2. **RegisterGates_cl.cpp** - Gate Operations
**Purpose**: Implements quantum gate operations on registers

```cpp
Gate Functions:
- HadamardR(int target) - Creates superposition
- XGateR(int target) - Bit flip
- YGateR(int target) - Y-axis rotation
- ZGateR(int target) - Phase flip
- SGateR(int target) - Phase gate (±i)
- TGateR(int target) - T gate (±e^iπ/4)
- CNOTgate(int control, int target) - Entanglement

Implementation Pattern:
1. Generate bit mask for target qubit
2. Iterate through all 2^n amplitudes
3. Pair amplitudes using mask
4. Apply gate mathematics
5. Normalize result
```

**Unique Approach**: Bitwise operations make gate logic explicit and teachable.

#### 3. **Gates_cl.cpp** - Gate Definitions
**Purpose**: Mathematical definitions and theory

```cpp
Content:
- Hadamard matrix: [[1, 1], [1, -1]] / √2
- Pauli matrices: X, Y, Z with exact values
- Phase gates: S (phase π/2), T (phase π/4)
- Single-qubit gate application templates
- Normalization verification
```

**Learning Resource**: Direct matrix forms aid theoretical understanding.
